...
impl EventSink for TauriSink {
    ...
    fn queue_item_updated(&self, index: usize, status: &str) {
        // Bridge for GUI-specific prompt events: the encoder signals prompts
        // via special status prefixes until BatchControl is implemented (Phase 3).
        if let Some(path) = status.strip_prefix("overwrite-prompt:") {
            let _ = self.app.emit("overwrite-prompt", path);
            return;
        }
        if let Some(filename) = status.strip_prefix("fallback-prompt:") {
